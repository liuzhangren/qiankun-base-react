...
class MenuPage extends Component {
  ...
  componentDidMount() {
    this.posTable.refresh({ roleType: 2 })
    this.forceUpdate();
  }
  ...
  userModalConfirm(data, type) {
    ...
    if (type === 'ADD') {
      ...
      const param = data.map(item => ({ roleId: obj.id, account: item.account }))
              ...
              table={
                {
                  ...
                  <Button className="noMain" icon="check-square" onClick={() => {
                    ...
                    const param = keys.map(item => ({ roleId: this.state.record.id, resourceId: item }))
                    const params = { data: param };
                    //
                    this.props.dispatch({
                  ...
                  rowSelected: record => {
                    ...
                    dispatch({
                      ...
                      if (res && res.code === 0) {
                        const keys = []; let arr = []; const
                          noChild = [];
                        arr = res.data.map(item => item.resourceId)
                        if (res.data.length > 0) {

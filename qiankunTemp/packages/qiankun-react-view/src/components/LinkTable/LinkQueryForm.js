...
import MonthRangePicker from '../Form/components/MonthRangePicker';
...
class LinkQueryForm extends Component {
  ...
  componentDidMount() {
    ...
    const func = () => {
      ...
      this.form && this.setState({ initHeight: this.form.offsetHeight, wrapWidth: form.offsetWidth })
      const This = this;
      window.addEventListener(
        'resize', () => {
          This.setState({ wrapWidth: form.offsetWidth }, () => console.log('hello', form.offsetWidth))
        })
    }
  ...
  getFields = (bool) => {
    ...
    const { expand, formItems, viewCount, yearTime, monthRangeTime } = this.state
    ...
    for (let i = 0; i < formItems.length; i += 1) {
      ...
      if (item.componentType === "optionSelect") {
        let { options = [], keys = "dictCode", label = "dictName", noAll } = JSON.parse(JSON.stringify(item.props));
        if (options && options.length && options[0][label] !== "全部" && !noAll) {
          const obj = {}
          obj[label] = "全部"
          obj[keys] = ""
          options.unshift(obj);
          ...
          <Col span={expand ? Math.floor(24 / row) : Math.floor(24 / (viewCount + 1))} key={i} style={{ display: i < count ? 'block' : 'none', marginBottom: "10px" }}>
            <div className={`ant-row ant-form-item ${style.formItem}`}>
              ...
              <div className={`ant-col ant-form-item-control-wrapper ant-form-item-control ${style.wrapperCol}`}>
                <YearPicker time={yearTime} key={this.state.yearKey} onChange={(time) => { this.setState(() => ({ yearTime: time })) }} />
              </div>
            </div>
          </Col>
        )
      } else if (item.componentType === "monthRangerPicker") {
        forms.push(
          <Col span={expand ? Math.floor(24 / row) : Math.floor(24 / (viewCount + 1))} key={i} style={{ display: i < count ? 'block' : 'none', marginBottom: "10px" }}>
            <div className={`ant-row ant-form-item ${style.formItem}`}>
              <div className={`ant-col ant-form-item-label ${style.lableCol}`} title={item.label}>
              ...
              <div className={`ant-col ant-form-item-control-wrapper ant-form-item-control ${style.wrapperCol}`}>
                <MonthRangePicker time={monthRangeTime} {...item.options} key={this.state.monthRangeKey} onChange={(time) => { this.setState(() => ({ monthRangeTime: time })) }} />
              </div>
            </div>
          </Col>
        )
  ...
  handleSearch = e => {
    const { formItems, yearTime, monthRangeTime } = this.state
  ...
  handleReset = () => {
    const { form } = this.props;
    form.resetFields();
    this.setState(() => ({ yearTime: null, monthRangeTime: [], yearKey: Math.random(), monthRangeKey: Math.random() }))
  };
  ...
  render() {
    ...
    let isIcon = false;
    const btnsSpan =
      formItems.length > viewCount ? Math.floor(24 / (viewCount + 1)) :
        (24 - Math.floor(formItems.length * 24 / (viewCount + 1)))
